...
Nav2ActionClient::Nav2ActionClient(rclcpp::Node::SharedPtr node) : m_actionLogger(node->get_logger()) {
	std::string ns(node->get_namespace());
	m_nav2Client = rclcpp_action::create_client<Nav2Action>(node, ns + "/navigate_through_poses");
	m_followClient = rclcpp_action::create_client<FollowPathAction>(node, ns + "/follow_path");
}
...
void Nav2ActionClient::SendNav2Goal(const Nav2Action::Goal& goal_msg, ResultCallback callback) {
	...
	send_goal_options.feedback_callback = [](GoalHandleNav2::SharedPtr,
	                                         const std::shared_ptr<const GoalHandleNav2::Feedback>) {};
	send_goal_options.result_callback = [&logger = m_actionLogger,
	                                     callback](const GoalHandleNav2::WrappedResult& result) {
		switch (result.code) {
...
void Nav2ActionClient::SendBlindGoal(const FollowPathAction::Goal& goal_msg, ResultCallback callback) {
	...
	send_goal_options.feedback_callback = [](GoalHandleFollowPath::SharedPtr,
	                                         const std::shared_ptr<const GoalHandleFollowPath::Feedback>) {};
	send_goal_options.result_callback = [&logger = m_actionLogger,
	                                     callback](const GoalHandleFollowPath::WrappedResult& result) {
		switch (result.code) {
			case rclcpp_action::ResultCode::SUCCEEDED:
				callback(true);
				break;
